...
description: >
  ...
  scan, and the per-repo publication rules (pushes = Mitch; dtgwg-zkp-mage pushes ride the
  G.1 proverb rite).
license: Apache-2.0
---
...
| **tome act** | `~/cityofmages/tomes/` | act inscription; bound by Mitch only | reflection PROPOSES, binding is human |
...
2. **First Person's read.** Master chronicles stay uncommitted until Mitch reads them; the
...
4. **Push = Mitch.** Reflections land on disk; publication is his act. For `dtgwg-zkp-mage`
...
When Mitch asks for a dream-loop sync: master → spellweb DREAM (retell as KG nodes/edges
